...
import { FileText, Image as ImageIcon, Download, Maximize2, X } from "lucide-react";
...
export function MessageList({ messages, loading, typingUsers = [], conversationId }: MessageListProps) {
  ...
  const { getMessageReactions } = useRealtimeReactions(conversationId, messageIds);
  ...
  return (
    <div className="flex-1 overflow-y-auto py-4 space-y-1">
      {messages.map((msg) => {
        ...
        const isEdited = new Date(msg.updatedAt || msg.createdAt).getTime() > new Date(msg.createdAt).getTime();
        ...
        return (
          ...
          >
            ...
            >
              ...
              {isEditing && isMine && msg.id === messages.find(m => m.id === msg.id)?.id ? (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className={cn("font-mono text-sm text-text leading-relaxed whitespace-pre-wrap break-words", isDeleted && "text-text-muted")}>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {reactionGroups.map((group) => (
                    ...
                    >
                      ...
                      <span className="text-[10px] leading-none">{group.count}</span>
                    </button>
                  ))}
                </div>
              )}
